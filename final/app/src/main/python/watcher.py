...
def init(files_dir):
    ...
    with _LOCK:
        ...
        """)
        dbcore.ensure_indexes(conn, "seen_files", [
            ("idx_seen_files_watch_id", "watch_id"),
        ])
        dbcore.ensure_indexes(conn, "watch_events", [
            ("idx_watch_events_watch_id", "watch_id"),
            ("idx_watch_events_detected_at", "detected_at"),
        ])
        conn.commit()
        conn.close()
    ...
    for w in watches:
        ...
        for path, _mtime in new_or_changed:
            for handler in EVENT_HANDLERS:
                try:
                    watch_timeout = config.get("watcher_handler_timeout_seconds", 30)
                except Exception:
                    watch_timeout = 30
                result = executor.run(
                    handler, args=(path,), timeout=watch_timeout,
                    name=getattr(handler, "__name__", "watch-handler"),
                )
                if result.timed_out:
                    logging.error(
                        "watcher handler '%s' timed out after %ss on %s - abandoned, scan continues",
                        result.name, result.elapsed_seconds, path,
                    )
                elif result.error:
                    logging.error(
