...
def init(files_dir):
    ...
    """)
    dbcore.ensure_indexes(conn, "rules", [("idx_rules_domain", "domain")])
    dbcore.ensure_indexes(conn, "extractions", [
        ("idx_extractions_domain", "domain"),
        ("idx_extractions_created_at", "created_at"),
    ])
    conn.commit()
    conn.close()
