...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Back button should behave like it does in a normal browser/app:
        // step back through the WebView's page history first, and only
        // let the system close/backgrounds the app once there's nowhere
        // left to go back to.
    ...
    private fun showSettingsMenu() {
        ...
        AlertDialog.Builder(this)
            ...
            .setItems(arrayOf("Reload", "Open Admin Panel", "Open Browser", "File Explorer", "Python Scripts", "Theme / UI Settings", "View Log", "View Error History", "View Screen Time", "Encrypted Backup Now", "Start LLM Engine", "Stop LLM Engine", "Reset App Data")) { _, which ->
                when (which) {
                    ...
                    4 -> webView.loadUrl(serverUrl + "scripts")
                    5 -> webView.loadUrl(serverUrl + "settings")
                    6 -> showLog()
                    7 -> showErrorHistory()
                    8 -> showScreenTime()
                    9 -> runEncryptedBackup()
                    10 -> startLlamaEngine()
                    11 -> stopLlamaEngine()
                    12 -> confirmReset()
                }
