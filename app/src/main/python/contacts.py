...
def init(files_dir):
    ...
    """)
    conn.execute("CREATE INDEX IF NOT EXISTS idx_links_contact ON links(contact_id)")
    conn.execute("CREATE INDEX IF NOT EXISTS idx_contacts_phone ON contacts(phone_norm)")
    conn.commit()
    conn.close()
